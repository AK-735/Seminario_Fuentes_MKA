...
ggplot(data = tendencia_anual, aes(x =ano_num, y = Casos_Nuevos))+
  geom_point(color = "green",size = 3)+
  geom_line(color = "red",size = 1.5) +
  geom_text(aes(label = Casos_Nuevos), 
...
corrleacion_tendencia_motivo <- tendencia_anual %>% 
  mutate(`Año de diagnóstico`= as.character(ano_num))%>%
  inner_join(
    Modotransmisioncompleto, by ="Año de diagnóstico"
  )%>%
  group_by(`Año de diagnóstico`)%>%
  mutate(
    Porcentaje = round((Casos / Casos_Nuevos)*100,2)
  )%>%
  ungroup() %>%
  select(
    Año = `Año de diagnóstico`,
    Casos_Totales_Anuales = Casos_Nuevos,
    `CCAA de Notificación`,
    `Modo de Transmisión`,
    `Casos del Modo`,
    `Porcentaje del Total (%)` = Porcentaje_Modo
  ) %>%
  arrange(Año, desc(`Porcentaje del Total (%)`))
datatable(correlacion_tendencia_motivo_completo, 
          caption = "Correlación: Tendencia Anual vs. Modo y CCAA de Transmisión",
          options = list(pageLength = 10, scrollX = TRUE))

print(correlacion_tendencia_motivo_completo)
