...
```{r, eval = TRUE,message=FALSE, warning = FALSE}
Modotransmisioncompleto <- read_ods(path = "Input/DATA/Modotranscompleto.ods") 
...
Aunque empleando motivosVIH, debido a que permite que se pueda visualizar con mayor claridad.
...
tendencia <- ggplot(data = tendencia_anual, aes(x =ano_num, y = Casos_Nuevos))+
  ...
  geom_text(aes(
    label = Casos_Nuevos), 
    vjust = -0.65, size = 3.5, color = "black") +
...
Como podemos ver en la siguiente gráfica se reafirma nuestro objetivo principal, mostrando como el motivo HSH denota una promitente superioridad en numero de nuevos casos que el resto. Por otra parte, podemos ver que a medida que pasan los años la incidencia del VIH se ha visto reducida debido a las campañas de concienciacion que tratan de prevenir dicho virus en la población
...
motivos_colores <- c(
    "HSH" = "red",           
    "Heterosexual" = "blue",   
    "Uso de Drogas IV (PID)" = "green", 
    "Materno-Infantil" = "black", 
    "Hemoderivados" = "orange",  
    "Transfusión" = "deeppink",    
    "Desconocido" = "pink"     
)

correlacion_tendencia_motivo <- correlacion_tendencia_motivo %>%
    mutate(`Año` = as.numeric(`Año`))

tendencia_motivos <- ggplot(data = correlacion_tendencia_motivo, aes(x = `Año`)) + 
  
  geom_smooth(aes(y = HSH, color = "HSH"),method = "lm", se = FALSE,linewidth = 1, alpha = 0.6) +
  geom_point(aes(y = HSH, color = "HSH"), size = 2, alpha = 0.6) +
  
  geom_smooth(aes(y = Heterosexual_H, color = "Heterosexual"),method = "lm",  se = FALSE,linewidth = 1, alpha = 0.6) +
  geom_point(aes(y = Heterosexual_H, color = "Heterosexual"), size = 2, alpha = 0.6) +
  
  geom_smooth(aes(y = PID_H, color = "Uso de Drogas IV (PID)"),method = "lm", se = FALSE,linewidth = 1, alpha = 0.6) +
  geom_point(aes(y = PID_H, color = "Uso de Drogas IV (PID)"), size = 2, alpha = 0.6) +
  
  geom_smooth(aes(y = Hemoderivados_H, color = "Hemoderivados"),method = "lm", se = FALSE, linewidth = 1, alpha = 0.6) +
  geom_point(aes(y = Hemoderivados_H, color = "Hemoderivados"), size = 2, alpha = 0.6) +
  
  geom_smooth(aes(y = Transfusion_H, color = "Transfusión"),method = "lm", se = FALSE, linewidth = 1, alpha = 0.6) +
  geom_point(aes(y = Transfusion_H, color = "Transfusión"), size = 2, alpha = 0.6) +
  
  geom_smooth(aes(y = Desconocido_H, color = "Desconocido"),method = "lm", se = FALSE, linewidth = 1, alpha = 0.6) +
  geom_point(aes(y = Desconocido_H, color = "Desconocido"), size = 2, alpha = 0.6) +

  geom_smooth(aes(y = Materno_Infantil_H, color = "Materno-Infantil"),method = "lm", se = FALSE, linewidth = 1, alpha = 0.6) +
  ...
  labs(
    title = "Evolución anual de motivos desencadenantes del VIH (2013-2023)",
    x = "Año de Diagnóstico",
