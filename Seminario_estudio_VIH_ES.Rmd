...
tendencia_parejas <- ggplot(data = parejas_vih, aes(x = `Parejas ultimo año`, y = ano, color = VIH, size = Frecuencia))+
  ...
  labs(
    ...
    subtitle = "El tamaño del punto indica el número de personas con esa respuesta",
...
tendencia <- ggplot(data = correlacion_tendencia_motivo, aes(x = Año, y = Casos_Nuevos))+
  geom_point(color = "green",size = 3)+
  geom_line(color = "red",size = 1.5) +
  geom_text(aes(
    label = Casos_Nuevos), 
...
tendencia_motivos <- ggplot(data = correlacion_tendencia_motivo, aes(x = `Año`)) + 
  ...
  geom_point(aes(y = Casos_Nuevos, color = "Total Casos Nuevos"), size = 4, alpha = 1.0) +
